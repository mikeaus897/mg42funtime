...
qboolean NET_OpenStreamedSocket(streamed_socket **ss_out, netadr_t *to)
{
	...
	if ((ss->socket = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1)
	{
	...
	if (ioctl(ss->socket, FIONBIO, &non_blocking) == -1)
	{
		Com_Printf("NET_OpenStreamedSocket ioctlsocket FIONBIO: %s\n", NET_ErrorString());
		NET_CloseStreamedSocket(ss);
		return qfalse;
	}
...
qboolean Sys_GetStreamedPacket(netadr_t *net_from, msg_t *msg)
{
	...
	for (i = 0; i < MAX_STREAMED_SOCKETS; ++i)
	{
		...
		ret = recv(ss->socket, msg->data, msg->maxsize, 0);
		if (ret == -1)
		{
			err = errno;
