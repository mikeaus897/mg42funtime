...
cvar_t  *com_logfileName;	// Name of log file. i.e. logs/rtcwconsole.log
cvar_t	*com_logfileMode;	// File open mode. 0 = Write (wb), 1 = Append (ab), 2 = Append Sync (ab + flush per write)
...
#endif
			switch (com_logfileMode->integer) {
			default:
				...
				break;
			case 1:
				FS_FOpenFileByMode(com_logfileName->string, &logfile, FS_APPEND);
				break;
			case 2:
				FS_FOpenFileByMode(com_logfileName->string, &logfile, FS_APPEND_SYNC);
				break;
			}
...
qboolean Com_LogFileExists() {
	return logfile != 0;
}

void Com_CloseLogFile() {
	if (Com_LogFileExists()) {
...
int Com_EventLoop( void ) {
	...
	while ( 1 ) {
		...
		case SE_STREAMED_PACKET:
			...
			if (com_sv_running->integer) {
				SV_StreamedPacketEvent(evFrom, &buf);
			} else {
...
Sets drop reload script to specified key
===============
*/
void Com_SetDropReload_f(void) {
	char key;

	if (Cmd_Argc() != 2) {
		Com_Printf("Usage: dropreload <key>\n");
		return;
	}

	key = *Cmd_Argv(1);
	Cbuf_AddText(va("bind %c \"+dropweapon; -dropweapon; +activate; -activate; -activate\"", key));
}
...
void Com_Init( char *commandLine ) {
	...
	com_logfile = Cvar_Get( "logfile", "0", CVAR_ARCHIVE);
	com_logfileName = Cvar_Get("logfileName", "rtcwconsole.log", CVAR_ARCHIVE);
	com_logfileMode = Cvar_Get("logfileMode", "0", CVAR_ARCHIVE);

	com_timescale = Cvar_Get( "timescale", "1", CVAR_CHEAT | CVAR_SYSTEMINFO );
	com_fixedtime = Cvar_Get( "fixedtime", "0", CVAR_CHEAT );
	com_showtrace = Cvar_Get( "com_showtrace", "0", CVAR_CHEAT );
	com_dropsim = Cvar_Get( "com_dropsim", "0", CVAR_CHEAT );
	com_latencysim = Cvar_Get("com_latencysim", "0", CVAR_CHEAT);
	...
	demo_paused = Cvar_Get("demo_paused", "0", 0);
	...
	Cmd_AddCommand( "dropreload", Com_SetDropReload_f );
