...
static void SV_AddEntitiesVisibleFromPoint( vec3_t origin, clientSnapshot_t *frame,
											snapshotEntityNumbers_t *eNums, qboolean portal, qboolean localClient, qboolean isBot  ) {
	int e, i;
	...
	for ( e = 0 ; e < sv.num_entities ; e++ ) {
		...
		// entities can be flagged to only be sent to bots
		if ( ent->r.svFlags & SVF_BROADCAST_BOTS ) {
			if ( !isBot ) {
				continue;
			}
		}
		...
		if ( ( playerEnt->s.eFlags & EF_VIEWING_CAMERA ) && !portal ) {
			if ( ent->r.svFlags & SVF_PORTAL ) {
				...
				SV_AddEntitiesVisibleFromPoint( ent->s.origin2, frame, eNums, qtrue, localClient, isBot );
			}
			continue;
		}
...
//----(SA)	end
	...
	SV_AddEntitiesVisibleFromPoint( org, frame, &entityNumbers, qfalse, client->netchan.remoteAddress.type == NA_LOOPBACK, client->netchan.remoteAddress.type == NA_BOT );
