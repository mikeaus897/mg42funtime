/*
===========================================================================
rtcwPub library with userinfo string exceeded challange part from iortw
...
void SV_DirectConnect( netadr_t from ) {
	...
	char        *denied;
        ...
        // added to get library compile without errors with userinfo challange
        char *ip;
	...
	if ( version != PROTOCOL_VERSION_1_0 && version != PROTOCOL_VERSION_1_4 ) {
	...
	for ( i = 0,cl = svs.clients ; i < sv_maxclients->integer ; i++,cl++ ) {
		if ( NET_CompareBaseAdr( from, cl->netchan.remoteAddress )
			...
			if ( ( svs.time - cl->lastConnectTime )
				...
				NET_OutOfBandPrint(NS_SERVER, from, "print\nReconnected too quickly. Please wait.\n");
...
//	Info_SetValueForKey( userinfo, "ip", ip );
	...
	if ( !NET_IsLocalAddress( from ) ) {
		...
		if ( i == MAX_CHALLENGES ) {
			NET_OutOfBandPrint( NS_SERVER, from, "print\nInvalid or outdated challenge. Please reconnect.\n" );
			return;
		}
		...
		if ( !Sys_IsLANAddress( from ) ) {
			if ( sv_maxPing->value && ping > sv_maxPing->value ) {
				NET_OutOfBandPrint( NS_SERVER, from, "print\nConnect ping too high. Try reconnecting.\n" );
	...
	if ( !newcl ) {
		if ( NET_IsLocalAddress( from ) ) {
			...
			NET_OutOfBandPrint( NS_SERVER, from, "print\nServer is full.\n" );
			Com_DPrintf( "Rejected a connection due to server being full.\n" );
			return;
		}
	}
...
// sswolf - dlrate port from iortcw > ospx, source: nihi
#if 0
/*
==================
	...
	if (!cl->download)
	{
		...
		if (pakptr)
		{
			*pakptr = '\0';
			...
			{
				const char* referencedPaks = FS_ReferencedPakNames(sv_forcedPk3Downloads->string);
...
static void SV_UpdateUserinfo_f( client_t *cl ) {
	...
	Q_strncpyz(userinfo, cl->userinfo, sizeof(cl->userinfo));
	Q_strncpyz(cl->userinfo, Cmd_Argv(1), sizeof(cl->userinfo));

	SV_UserinfoChanged( cl );

	// Don't send duplicate userinfo packets to the game
	if (Q_strcasecmp(userinfo, cl->userinfo) == 0)
	{
		return;
	}
...
char *ProtocolToString(int protocol)
{
	switch (protocol)
	{
	case PROTOCOL_VERSION_1_4:
		return "1.4";
	case PROTOCOL_VERSION_1_3:
		return "1.3";
	default:
		return "1.0";
	}
