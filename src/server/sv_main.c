...
cvar_t	*sv_dailyRestartHour;

// Nobo - global stats
cvar_t *sv_globalStats;
cvar_t *sv_globalStatsToken;
cvar_t *sv_globalStatsServerName;
cvar_t *sv_globalStatsServerPath;
cvar_t *sv_globalStatsPort;

// Nobo - external ip
cvar_t *sv_ipExternal;
cvar_t *sv_getExternalIpDomain;
cvar_t *sv_getExternalIpServerIp;

// Nobo - forced pk3 downloads
cvar_t *sv_forcedPk3Downloads;
...
void SVC_Info( netadr_t from ) {
	...
	Info_SetValueForKey(infostring, "protocol", va("%i", PROTOCOL_VERSION_1_0));
	NET_OutOfBandPrint( NS_SERVER, from, "infoResponse\n%s", infostring );

	// and 1.4
	Info_SetValueForKey(infostring, "protocol", va("%i", PROTOCOL_VERSION_1_4));
	NET_OutOfBandPrint(NS_SERVER, from, "infoResponse\n%s", infostring);
}
...
// L0 - So it can optionally be tested localy if need it..
#ifndef _DEBUG
...
/******** L0 - rate fix from openWolf Ends here ********/
...
void SV_StreamedPacketEvent(netadr_t from, msg_t *msg)
{
	...
	// remove extra junk that chunked transfer adds to body
	if (response->chunked)
	{
		response->body = strstr(response->body, "\r\n") + 2;
	}
	...
	if (response->code == 200)
	{
		...
		{
			if (Q_stricmp(NET_AdrToString(from), sv_getExternalIpServerIp->string) == 0)
			{
				Cvar_Set("sv_ipExternal", response->body);
				Com_Printf("Get external ip response from %s - %s\n", NET_AdrToString(from), response->body);
			}
			else
			{
				Com_Printf("SV http packet from %s:\n%s\n", NET_AdrToString(from), response->body);
			}
		}
		else
		{
			Com_Printf("SV http packet from %s - missing body\n", NET_AdrToString(from));
		}
	}
	else
	{
		Com_Printf("SV http packet from %s - unexpected response code: %i\n", NET_AdrToString(from), response->code);
	}
}
...
void SV_Frame( int msec ) {
	...
	// restart server at given hour military time; 0-23 (0 == midnight).
	// this is useful if you want to avoid the two restarts below,
	// which will happen at a random hour and possibly disrput players.
	// with this you can at-least choose an hour when server activity is low to none.
	if (sv_dailyRestartHour->integer >= 0) {
		Com_RealTime(&time);
		if (time.tm_hour == sv_dailyRestartHour->integer) {
			if (!svs.dailyRestarted) {
				SV_Restart( "Daily server restart triggered. Please reconnect." );
				svs.dailyRestarted = qtrue;
				return;
			}
		} else {
			svs.dailyRestarted = qfalse;
		}
	}
	...
	if ( svs.time > 0x70000000 ) {
		SV_Restart( "Restarting server due to time wrapping. Please reconnect." );
		return;
	}
	// this can happen considerably earlier when lots of clients play and the map doesn't change
	if ( svs.nextSnapshotEntities >= 0x7FFFFFFE - svs.numSnapshotEntities ) {
		SV_Restart( "Restarting server due to numSnapshotEntities wrapping. Please reconnect." );
		return;
	}
